<script lang="ts">
  ...
  async function loadRunData(runId: string) {
    ...
    try {
      ...
      if (selectedProvider) {
        competitors = competitors.filter(c => c.llm_providers && c.llm_providers.includes(selectedProvider!.name))
      } else {
        const groups = new Map()
        competitors.forEach(c => {
          const key = c.business_name
          if (!groups.has(key)) {
            groups.set(key, { ...c, allRanks: [...c.raw_ranks], allProviders: [...c.llm_providers], totalAppearances: c.appearances_count, totalProviderAttempts: c.total_attempts })
          } else {
            const g = groups.get(key)
            g.allRanks.push(...c.raw_ranks)
            c.llm_providers.forEach((p: string) => { if (!g.allProviders.includes(p)) g.allProviders.push(p) })
        ...
        competitors = Array.from(groups.values()).map(g => {
          ...
          competitorData?.forEach(c => { if (c.business_name === g.business_name) { providerAverages.push(c.average_rank); providerWeighted.push(c.weighted_score) } })
            const avg = providerAverages.reduce((s, v) => s + v, 0) / providerAverages.length
          const weighted = providerWeighted.reduce((s,v)=> s+v,0) / providerWeighted.length
          return { ...g, average_rank: Number(avg.toFixed(2)), best_rank: Math.min(...g.allRanks), worst_rank: Math.max(...g.allRanks), appearances_count: g.totalAppearances, total_attempts: g.totalProviderAttempts, weighted_score: Number(weighted.toFixed(2)), llm_providers: g.allProviders, raw_ranks: g.allRanks }
        })
      }
  ...
  async function selectRun(runId: string) { if (runId !== selectedRunId) { selectedRunId = runId; await loadRunData(runId) } }
  function goBack() { goto('/') }
  function formatDate(d: string) { return new Date(d).toLocaleString('en-US', { year:'numeric', month:'short', day:'numeric', hour:'2-digit', minute:'2-digit'}) }
</script>
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-8">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
    {#if error}
      ...
      <button onclick={() => goBack()} class="mt-4 text-sm font-medium text-blue-600 hover:text-blue-700 cursor-pointer">← Back to Dashboard</button>
    {:else if query}
      <!-- Header -->
      <div class="flex items-start justify-between flex-col md:flex-row gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
          <button class="px-3 py-1.5 rounded-full text-xs font-medium border cursor-pointer transition-colors {selectedProvider === null ? 'bg-blue-600 text-white border-blue-600' : 'border-slate-300 text-slate-600 hover:bg-slate-100'}" onclick={() => onProviderChange(null)}>All</button>
          {#each llmProviders as provider}
            <button class="px-3 py-1.5 rounded-full text-xs font-medium border cursor-pointer transition-colors flex items-center gap-1 {selectedProvider?.id === provider.id ? 'bg-blue-600 text-white border-blue-600' : 'border-slate-300 text-slate-600 hover:bg-slate-100'}" onclick={() => onProviderChange(provider)}>
      ...
      <div class="grid gap-6 lg:grid-cols-12">
        ...
        <Card padding="p-6" custom="lg:col-span-5">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-sm font-semibold text-slate-700 tracking-wide">Analysis Runs</h3>
            <span class="text-[11px] text-slate-500">{analysisRuns.length} total</span>
          </div>
          {#if analysisRuns.length > 0}
            <div class="space-y-2">
              {#each analysisRuns as run (run.id)}
                <button type="button" class="w-full text-left p-3 rounded-lg border transition-colors cursor-pointer {selectedRunId === run.id ? 'bg-blue-50 border-blue-400' : 'bg-white border-slate-200 hover:border-slate-300'}" onclick={() => selectRun(run.id)}>
        ...
        <div class="lg:col-span-7">
          {#if selectedRunId && filteredRankingResults && filteredRankingResults.length > 0}
            <Card padding="p-6">
              <h3 class="text-sm font-semibold text-slate-700 mb-4">LLM Ranking Attempts</h3>
          ...
          {:else if selectedRunId}
            <Card padding="p-10" custom="flex items-center justify-center">
              <p class="text-sm text-slate-500">{selectedProvider ? 'No LLM results for selected provider and run' : 'No LLM results for selected run yet'}</p>
            </Card>
          {:else}
            <Card padding="p-10" custom="flex items-center justify-center">
              <p class="text-sm text-slate-500">Select a run to view provider attempts</p>
            </Card>
          {/if}
        </div>
      </div>
  ...
  <div class="-mt-2"> <!-- space-y-8 adds 2rem; -mt-4 offsets by 1rem => effective 1rem gap -->
        ...
        {:else if selectedRunId && competitorRankings.length > 0}
          <Card padding="p-6">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-sm font-semibold text-slate-700">Competitor Rankings</h3>
              <span class="text-[11px] text-slate-500">{competitorRankings.length} businesses</span>
            </div>
            <CompetitorRankingsTable {competitorRankings} />
          </Card>
        {:else if selectedRunId}
          <Card padding="p-10" custom="text-center">
            <p class="text-sm text-slate-500">{selectedProvider ? 'No competitor rankings for selected provider and run.' : 'No competitor rankings for this run yet.'}</p>
          </Card>
        {:else if analysisRuns.length === 0}
          <Card padding="p-10" custom="text-center">
            <p class="text-sm text-slate-500">No analysis runs found for this query. Run an analysis to see results.</p>
          </Card>
        {/if}
      </div>
    {:else}
