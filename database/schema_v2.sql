...
-- Online sources discovered via LLM grounding (Gemini Google Search) after each run.
-- Collected for the target product + competitors ranked above the target.
CREATE TABLE public.source_citations (
  ...
  url              text,
  title            text,
  snippet          text,   -- brief context snippet from grounding response
...
-- source_citations
CREATE INDEX idx_source_citations_run_measurement ON public.source_citations(analysis_run_id, measurement_id);
CREATE INDEX idx_source_citations_product         ON public.source_citations(analysis_run_id, product_name);
