  ...
  <nav class="max-w-7xl mx-auto h-12 px-3 sm:px-5 lg:px-6 flex items-center justify-between">
    <div class="flex items-center gap-4">
      <a href="/" class="group inline-flex items-center gap-1.5 font-semibold text-slate-800 tracking-tight text-[15px] cursor-pointer">
        <span class="inline-flex h-7 w-7 items-center justify-center rounded-md bg-gradient-to-br from-blue-600 to-indigo-600 text-white font-bold text-xs shadow-sm">RL</span>
    ...
    <button class="md:hidden inline-flex items-center justify-center h-8 w-8 rounded-md border border-slate-300 bg-white text-slate-600 hover:bg-slate-50 focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 focus-visible:ring-offset-white cursor-pointer" onclick={toggle} aria-label="Toggle navigation" aria-expanded={mobileOpen} aria-controls="mobile-nav">
      ...
      {:else}
        <a href="/signin" class="hidden sm:inline-flex items-center justify-center px-3 py-1.5 rounded-md bg-blue-600 text-white text-xs font-medium shadow-sm hover:bg-blue-700 transition-colors cursor-pointer">Sign In</a>
        <a href="/signin" class="sm:hidden inline-flex items-center justify-center h-8 w-8 rounded-md bg-blue-600 text-white font-medium shadow-sm hover:bg-blue-700 cursor-pointer" aria-label="Sign in">
    ...
    <div id="mobile-nav" class="md:hidden border-t border-slate-200 bg-white/95 backdrop-blur-sm" role="navigation" aria-label="Mobile navigation">
      <div class="px-3 py-3">
        {#if session}
          <UserDropdown {session} />
        {:else}
          <a href="/signin" class="inline-flex w-full items-center justify-center px-3 py-2 rounded-md bg-blue-600 text-white text-xs font-medium shadow-sm hover:bg-blue-700 cursor-pointer" onclick={close}>Sign In</a>
        {/if}
      </div>
    </div>
  {/if}
